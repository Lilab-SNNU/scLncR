...
#' Load a featureCounts-derived gene by sample matrix
#'
#' @description
#' Reads a tab-delimited count matrix generated by the Smart-seq2 count branch.
#' The expected minimal format is one gene identifier column followed by raw
#' count columns. If featureCounts annotation columns are present, they are
#' automatically removed before constructing the matrix.
#'
#' @param counts_matrix Path to a TSV count matrix.
#' @return A numeric matrix with genes as rows and samples/cells as columns.
#' @export
load_featurecounts_matrix <- function(counts_matrix) {
    if (!file.exists(counts_matrix)) {
        stop("featureCounts matrix file not found: ", counts_matrix)
    }
    df <- read.delim(counts_matrix, check.names = FALSE, stringsAsFactors = FALSE)
    if (nrow(df) == 0 || ncol(df) < 2) {
        stop("featureCounts matrix must contain one gene column and at least one count column: ", counts_matrix)
    }

    gene_col_candidates <- c("gene_id", "Geneid", "GeneID", "gene", "Gene")
    gene_col <- intersect(gene_col_candidates, colnames(df))
    gene_col <- if (length(gene_col) > 0) gene_col[1] else colnames(df)[1]

    annotation_cols <- intersect(c(gene_col, "Chr", "Start", "End", "Strand", "Length"), colnames(df))
    count_cols <- setdiff(colnames(df), annotation_cols)
    if (length(count_cols) == 0) {
        stop("No count columns detected in featureCounts matrix: ", counts_matrix)
    }

    counts_df <- df[, count_cols, drop = FALSE]
    counts_df[] <- lapply(counts_df, function(x) suppressWarnings(as.numeric(x)))
    if (anyNA(as.matrix(counts_df))) {
        stop("Non-numeric values detected in count columns of: ", counts_matrix)
    }
...
#' Create a Seurat object from a raw count matrix
#'
#' @description
#' Builds a Seurat object from a gene by sample/cell matrix and optionally
#' attaches sample metadata. This is used by the Smart-seq2 featureCounts
#' matrix input path.
#'
#' @param counts_mat Numeric count matrix with genes as rows.
#' @param project_name Seurat project name.
#' @param samples_info Optional tab-delimited metadata file.
#' @return A Seurat object.
#' @export
create_seurat_from_count_matrix <- function(counts_mat, project_name = "scLncR", samples_info = "") {
    if (is.null(colnames(counts_mat)) || any(!nzchar(colnames(counts_mat)))) {
        colnames(counts_mat) <- paste0(project_name, "_", seq_len(ncol(counts_mat)))
        warning("Count matrix had missing sample/cell names; generated fallback column names.")
    }

    seu_obj <- CreateSeuratObject(counts = counts_mat, project = project_name)
    seu_obj$orig.ident <- colnames(counts_mat)

    if (nzchar(samples_info) && file.exists(samples_info)) {
        meta_df <- read.table(samples_info, header = TRUE, sep = "\t", stringsAsFactors = FALSE, check.names = FALSE)
        sample_col_candidates <- c("sample_id", "sample", "Sample", "SampleID", "cell_id", "cell")
        sample_col <- intersect(sample_col_candidates, colnames(meta_df))
        if (length(sample_col) > 0) {
            sample_col <- sample_col[1]
            row_idx <- match(colnames(seu_obj), as.character(meta_df[[sample_col]]))
            matched <- !is.na(row_idx)
            if (any(matched)) {
                meta_use <- meta_df[row_idx[matched], setdiff(colnames(meta_df), sample_col), drop = FALSE]
                rownames(meta_use) <- colnames(seu_obj)[matched]
                seu_obj <- AddMetaData(seu_obj, metadata = meta_use)
            }
            if (any(!matched)) {
                warning(
                    "samples_info did not match all matrix columns. Unmatched columns: ",
                    paste(colnames(seu_obj)[!matched], collapse = ", ")
                )
            }
        } else {
            warning("samples_info has no recognized sample column; expected one of: ", paste(sample_col_candidates, collapse = ", "))
        }
    }
...
load_sc <- function(data_dir="", project_name="", mt_name="", min.RNAs=100, max.RNAs=7000, pct.mt=10, lnc_name="", nfeatures=6000, input_format="10x", counts_matrix="", samples_info=""){
    cat("=============== Data loading ...... \n")
    print(ifelse(input_format == "featurecounts_matrix", counts_matrix, data_dir))
    if (input_format == "featurecounts_matrix") {
        data <- load_featurecounts_matrix(counts_matrix)
        seu_obj <- create_seurat_from_count_matrix(data, project_name = project_name, samples_info = samples_info)
    } else {
...
data_preprocess <- function(seu_obj, lnc=TRUE, lnc_name=lnc_name, nfeatures=nfeatures, dims=15, resolution=0.8, output_dir="", colour=colour){
    ...
    dims_requested <- dims
    dims <- min(as.integer(dims), max(1L, ncol(seu_obj) - 1L), max(1L, length(VariableFeatures(seu_obj))))
    if (dims < dims_requested) {
        warning(sprintf("Reducing dims from %s to %s based on available cells/features.", dims_requested, dims))
    }
    ...
    seu_obj <- RunPCA(seu_obj, features = VariableFeatures(object = seu_obj), npcs = dims)

    # Get a small preview of PCA loadings when enough dimensions are available.
    pca_loadings <- Loadings(object = seu_obj[["pca"]])
    pca_rows <- seq_len(min(5, nrow(pca_loadings)))
    pca_cols <- seq_len(min(5, ncol(pca_loadings)))
    if (length(pca_rows) > 0 && length(pca_cols) > 0) {
        t(pca_loadings[pca_rows, pca_cols, drop = FALSE])
        t(Loadings(object = seu_obj, reduction = "pca")[pca_rows, pca_cols, drop = FALSE])
    }
    ...
    try(VizDimLoadings(seu_obj, dims = 1:min(5, dims)), silent = TRUE)
    
    plot <- DimPlot(seu_obj, reduction = "pca")
    ggsave('DimPlot.pdf', plot=plot, width = 8, height = 6)
    
    pdf('DimHeatmap.pdf', width = 10, height = 8)
    DimHeatmap(seu_obj, dims = 1:min(15, dims), cells = min(500, ncol(seu_obj)), balanced = TRUE)
    dev.off()
...
run_dataProcess <- function(user_args, script_dir) {
    ...
    if (input_format == "featurecounts_matrix") {
        message("Using featureCounts matrix input for dataProcess: ", counts_matrix)
        seu_obj <- load_sc(data_dir="",
        ...
        raw_rds <- paste0(project_name, "_seu_obj_raw.rds")
        if (file.exists(raw_rds)) {
            file.rename(raw_rds, file.path(rawdata_dir, basename(raw_rds)))
        }
        saveRDS(seu_obj, file=file.path(rawdata_dir, "/seu_obj_inte_raw.rds"))
    } else {
        ...
        if (length(count_dirs) == 0) {
            stop("No filtered_feature_bc_matrix directories found under counts_dir: ", counts_dir,
                 "\nCurrent dataProcess expects 10x/Cell Ranger-style matrix directories when input_format='10x'.")
        }
